...
const imageRec = new Clarifai.App({ apiKey: config.clarifaiAPIKey });
...
module.exports = (pic, habits) => {
  return imageRec.models
    .predict(Clarifai.GENERAL_MODEL, pic)
    .then(data => {
      let imageRecData = data;
      imageRecData.tags = data.outputs[0].data.concepts.map(t =>
        t.name.toLowerCase()
      );
