...
class IndividualHabit extends Component {
  ...
  _setPrivate() {
    this.state.private
      ? Alert.alert("Habit hidden from other users")
      : Alert.alert("Habit will display to other users");
  ...
  render() {
    ...
    let dateStrings = this.props.habitProps.dates
      .sort((a, b) => new Date(a.date.toString()) - new Date(b.date.toString()))
    ...
    let totalDays = Math.max(
      ...
      this.props.habitProps.dates.length,
      1
    );
    ...
    let weeklyAvg =
      Math.round(this.props.habitProps.dates.length / totalDays * 7) || 0;
    let longestStreak = 0;
    let habitScore = 0;
    ...
    if (allDates[allDates.length - 1] && allDates[allDates.length - 2]) {
      currentStreak =
        allDates[allDates.length - 1][2] > allDates[allDates.length - 2][2]
          ? allDates[allDates.length - 1][2]
          : allDates[allDates.length - 2][2];
    } else {
    ...
    longestStreak =
      longestStreak === 1 ? longestStreak + " day" : longestStreak + " days";
    ...
    let images = this.props.habitProps.dates
      ? this.props.habitProps.dates.map(d => d.picture)
      : ["No Pictures Yet"];
    if (images.length > 6) {
        ...
        <ScrollView>
          <View>
            <View style={[styles.stats, { paddingTop: 10 }]}>
              ...
              <Text>
                {this.props.habitProps.dates.length}/{totalDays} ({Math.floor(this.props.habitProps.dates.length / totalDays * 100)}%)
              </Text>
            </View>
            ...
            <View style={styles.container}>
              <View style={styles.habitImages}>
                ...
                {photos.map(image => {
                  return (
                    <TouchableOpacity
                      onPress={() => this.props.showUserHabitPhoto(image)}
                    >
                      <Image
                    ...
                    onPress={() =>
                      Alert.alert(
                        ...
                        [
                          {
                            text: "Cancel",
                            onPress: () => console.log("Canceled Habit Delete!")
                          },
                          {
                            text: "OK",
                            onPress: () => {
