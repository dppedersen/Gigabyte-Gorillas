...
const Day = ({ day, howMany, habitProps, allHabits, onPressItem }) => {
  ...
  if (day.default === true) {
    image = (
      <View
        style={{ opacity: 0.5, alignItems: "center", justifyContent: "center" }}
      >
        <Image
          style={[
            styles.dayContainerImage,
            {
              width: imageWidth,
              height: imageHeight,
    ...
    if (
      day.picture ===
      "https://s3.us-east-2.amazonaws.com/tgoc99habit/bTyogRbpc.jpg"
    ) {
      image = (
        <View
          style={{
      ...
      image = (
        <View>
          <Image
            style={[
              styles.dayContainerImage,
              { opacity: 1, width: imageWidth, height: imageHeight }
            ]}
  ...
  return (
    <TouchableOpacity
      style={[styles.dayContainer, { width: imageWidth }]}
      onPress={() => onPressItem(day, habitProps, allHabits)}
    >
      {image}
      <Text style={styles.dayOfWeekTitle}>
