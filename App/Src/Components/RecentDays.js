...
const RecentDays = ({
  ...
  let dateStrings = habitProps.dates.map(d => [
  ...
  allDates = allDates
    .sort((a, b) => new Date(b[0].toString()) - new Date(a[0].toString()))
    .map(d => {
      ...
      if (indexDate > -1) {
        ...
        day = {
          ...
          date: d[0].toString(),
          picture:
            "https://thumb1.shutterstock.com/display_pic_with_logo/162265/276671360/stock-photo-motivational-quote-to-create-future-on-nature-abstract-background-276671360.jpg",
          default: true
        };
        pic = false;
      }
      ...
      return (
        <Day
          key={day.id}
          day={day}
          howMany={allDates.length}
          allHabits={allHabits}
          habitProps={habitProps}
          onPressItem={pic ? onPressPic.bind({}) : onPressNoPic.bind({})}
        />
      );
    });
