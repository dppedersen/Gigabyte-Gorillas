...
class Auth extends Component {
  ...
  render() {
    let enableAuth =
      this.state.username.length >= 3 && this.state.password.length >= 6 ? "" : "disabled";
    let alt = this.state.route === "Login" ? "SignUp" : "Login";
    let altText =
      this.state.route === "SignUp" ? "Already Have an Account? Login" : "Sign Up For an Account";
...
const mapDispatchToProps = dispatch => {
  return {
    onLogin: (username, password, email, route) => {
      dispatch(auth(username, password, email, route));
    },
